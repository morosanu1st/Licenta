			...
			double qme, double qmi, double qse, double qsi, double c, int np, int m)
{
	...
	aux_K = 0.0;
	double c_square = c * c;
	double qse_qme = qse / qme;
	double qsi_qmi = qsi / qmi;
	// electrons
	for (p = 0; p < np; p++)
	{
		gamma_p = 1.0 / sqrt(1.0 - (vx[p] * vx[p] + vy[p] * vy[p] + vz[p] * vz[p]) / c_square);
		aux_K = aux_K + (gamma_p - 1) * qse_qme;
	}
	...
	for (i = 2; i <= m - 3; i++)
		aux_E = aux_E + (ex[i] + ex0) * (ex[i] + ex0) + (ey[i] + ey0) * (ey[i] + ey0) + (ez[i] + ez0) * (ez[i] + ez0);
	...
	// se adauga si contributia externa; in versiunea curenta campul extern este uniform
	for (i = 2; i <= m - 3; i++)
		aux_B = aux_B + bx0 * bx0 + (by[i] + by0) * (by[i] + by0) +(bz[i] + bz0)*(bz[i] + bz0);
