...
int main(int argc, char **argv)
{
	...
	// // read the particles file
	// f_prtl = fopen("prtl_10.d", "r");
	// fread(&n1, sizeof(int), 1, f_prtl);
	// fread(&np1, sizeof(int), 1, f_prtl);
	// fclose(f_prtl);
	// if(n1==n&&np==np1){
	// 	cout<<endl<<endl<<"ok"<<endl<<endl;
	// 	return 0;
	// }
	// fread(&x1[0], sizeof(double), 2 * np, f_prtl);
	// fread(&vx1[0], sizeof(double), 2 * np, f_prtl);
	// fread(&vy1[0], sizeof(double), 2 * np, f_prtl);
	// fread(&vz1[0], sizeof(double), 2 * np, f_prtl);
	// fclose(f_prtl);
